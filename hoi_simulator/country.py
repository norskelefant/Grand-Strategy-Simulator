...
class Country: 
    def __init__(self, name, states, tiles, resources, free_civs, civs_used_on_consumer_goods,  free_mils, free_dockyards, construction, base_ic, base_stability, stability_modifier, economy_law, base_war_support, war_support_modifier, political_power, population, fuel, command_power, convoys, army_exp, navy_exp, air_exp, ideology, democratic_support, non_aligned_support, communist_support, fascist_support, at_war, countries_at_war_with, research_slots, has_researched, can_research, trade_law, conscription_law, advisors, possible_advisors, industrial_concern, possible_industrial_concerns, theorist, possible_theorists, chief_of_army, possible_chiefs_of_army, chief_of_navy, possible_chiefs_of_navy, chief_of_air_force, possible_chiefs_of_air_force, high_commanders, possible_high_commanders, leader, possible_leaders, focus_tree, focuses_done, focuses_that_can_be_done, national_spirits, modifiers, events_gotten, possible_events, intelligence_agency, full_added_bonuses, surrender_progress, number_of_divisions, number_of_planes, number_of_ships, victory_points_in_own_control, victory_points_in_enemy_control): 
        ...
        self.surrender_progress = surrender_progress
        self.number_of_divisions = number_of_divisions
        self.number_of_planes = number_of_planes
        self.number_of_ships = number_of_ships
        self.victory_points_in_own_control = victory_points_in_own_control
        self.victory_points_in_enemy_control = victory_points_in_enemy_control
    ...
    #25% * floor((1+10%)*(1-14.4%) * 100) / 100 = 0.235
    #https://store.steampowered.com/news/posts/?appids=394360&enddate=1691688053&feed=steam_community_announcements
    ...
    def set_economy_law(self, new_law): 
        old_law = self.get_economy_law()
        has_switched = False
        if new_law == economy_laws.Economy_laws.CIVILIAN_ECONOMY and has_switched == False:
            self.economy_law = POSSIBLE_ECONOMY_LAWS["Civilian_economy"]
            self.remove_from_full_added_bonuses(old_law)
            self.add_to_full_added_bonuses(self.get_economy_law())
            has_switched = True
        elif new_law == economy_laws.Economy_laws.EARLY_MOBILIZATION and has_switched == False:
            self.economy_law = POSSIBLE_ECONOMY_LAWS["Early_mobilization"]
            self.remove_from_full_added_bonuses(old_law)
            self.add_to_full_added_bonuses(self.get_economy_law())
            has_switched = True
        elif new_law == economy_laws.Economy_laws.PARTIAL_MOBILIZATION and has_switched == False: 
            self.economy_law = POSSIBLE_ECONOMY_LAWS["Partial_mobilization"]
            has_switched = True
        elif new_law == economy_laws.Economy_laws.WAR_ECONOMY and has_switched == False: 
            self.economy_law = POSSIBLE_ECONOMY_LAWS["War_economy"]
            self.remove_from_full_added_bonuses(old_law)
            self.add_to_full_added_bonuses(self.get_economy_law())
            has_switched = True
        elif new_law == economy_laws.Economy_laws.TOTAL_MOBILIZATION and has_switched == False: 
            self.economy_law = POSSIBLE_ECONOMY_LAWS["Total_mobilization"]
            self.remove_from_full_added_bonuses(old_law)
            self.add_to_full_added_bonuses(self.get_economy_law())
            has_switched = True
    ...
    def set_trade_law(self, new_law): 
        old_law = self.get_trade_law()
        has_switched = False
        if new_law == trade_laws.Trade_laws.FREE_TRADE and has_switched == False:
            self.economy_law = POSSIBLE_TRADE_LAWS["Free_trade"]
            self.remove_from_full_added_bonuses(old_law)
            self.add_to_full_added_bonuses(self.get_trade_law())
            has_switched = True
        elif new_law == trade_laws.Trade_laws.EXPORT_FOCUS and has_switched == False:
            self.economy_law = POSSIBLE_TRADE_LAWS["Export_focus"]
            self.remove_from_full_added_bonuses(old_law)
            self.add_to_full_added_bonuses(self.get_trade_law())
            has_switched = True
        elif new_law == trade_laws.Trade_laws.LIMITED_EXPORTS and has_switched == False: 
            self.economy_law = POSSIBLE_TRADE_LAWS["Limited_exports"]
            self.remove_from_full_added_bonuses(old_law)
            self.add_to_full_added_bonuses(self.get_trade_law())
            has_switched = True
        elif new_law == trade_laws.Trade_laws.CLOSED_ECONOMY and has_switched == False: 
    ...
    def create_default_bonuses_map(self):
        defaults = {
            ...
            modifier_types.Modifier_types.THEORIST_COST: 0.0,
            modifier_types.Modifier_types.INDUSTRIAL_CONCERN_COST: 0.0,
            modifier_types.Modifier_types.CHIEF_OF_ARMY_COST: 0.0,
            modifier_types.Modifier_types.CHIEF_OF_NAVY_COST: 0.0,
            modifier_types.Modifier_types.CHIEF_OF_AIR_FORCE_COST: 0.0,
            modifier_types.Modifier_types.HIGH_COMMANDER_COST: 0.0,
